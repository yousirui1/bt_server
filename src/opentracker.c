...
int start_track_server(int port)
{
...
"w:"
  ...
  //if( !scan_ushort( optarg, &tmpport)) { port; exit( 1 ); }
   // 	ot_try_bind( serverip, tmpport, FLAG_TCP ); bound++;
	

  /* Bind to our default tcp/udp ports */
  if( !bound) {
    ot_try_bind( serverip, port, FLAG_TCP );
    ot_try_bind( serverip, port, FLAG_UDP );
  }
