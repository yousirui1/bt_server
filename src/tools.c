...
int get_netcard_status()
{
    ...
    while ((retval = read(fd, buf, 1024)) > 0)  
    {   
...
unsigned long ulrand(void)
{
    ...
    return (
        (((unsigned long)rand() << 24) & 0xFF000000ul) | (((unsigned long)rand() << 12) & 0x00FFF000ul) | (((unsigned long)rand()) & 0x00000FFFul));
}
/* 随机一个64位 */
unsigned long long ullrand(void)
{
    return (
        (((unsigned long long)ulrand()) << 32) | ((unsigned long long)ulrand()));
}
